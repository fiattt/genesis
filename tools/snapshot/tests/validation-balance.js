...
module.exports = ( state, callback ) => {
  db.sequelize
    .query(`SELECT address FROM wallets WHERE balance_wallet>=${config.snapshot_minimum_balance} and register_error="balance_insufficient"`, {type: db.sequelize.QueryTypes.SELECT})
    .then(anomalies => {
      if(anomalies.length)
